...
export class CreateSessaoDto {
    ...
    @ApiProperty({ description: 'Nome do filme da sessão' })
    filme: string;
    ...
    @ApiProperty({ description: 'Data da sessão no formato YYYY-MM-DD' })
    date: string;
    ...
    @ApiProperty({ description: 'Horário da sessão no formato HH:MM:SS' })
    horario: string;

    @IsString()
    @IsNotEmpty({ message: 'A sala não pode estar vazia' })
    @ApiProperty({ description: 'Identificação da sala onde a sessão ocorrerá' })
    sala: string;

    @IsNumber({ maxDecimalPlaces: 2 }, { message: 'O preço deve ser um número válido' })
    @IsNotEmpty({ message: 'O preço não pode estar vazio' })
    @ApiProperty({ description: 'Preço do ingresso para a sessão' })
    preco: number;

    @Min(16, { message: 'O total de assentos deve ser pelo menos 16' })
    @ApiProperty({ description: 'Total de assentos disponíveis na sessão', minimum: 16 })
